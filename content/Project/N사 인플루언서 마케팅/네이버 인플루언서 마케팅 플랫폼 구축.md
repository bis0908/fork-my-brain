...
특정 키워드에서 **상위노출을 유지하는 인플루언서는 극소수**.

광고주는 이들을 찾기 어렵고, 인플루언서는 광고 기회를 놓침.
...
> **구현 현황 메모 (2026-06 코드 검증)**: 위 'FOR UPDATE 락 분산 상호배제' 및 '트랜잭션 ROLLBACK 자동 복구'는 설계 의도이며, 실제 코드는 해당 쿼리를 트랜잭션 없이 `pool.query`(autocommit)로 실행해 행 락이 즉시 해제되고 복구 트랜잭션이 빈 트랜잭션 상태다. 실질 중복 방지는 단일 프로세스 내 순차 처리 플래그에 의존하며, 고아 'sending' 행 회수 로직은 미구현. 이력서·면접 어필 시 이 한계를 전제로 표현할 것.
...
| 실시간성      | 최대 3 초 지연                  | 즉시               |
| 비용        | 없음                         | 추가 서버 비용         |
| **선택 이유** | 마케팅 이메일의 3 초 지연은 비즈니스에 무의미 |                  |
...
- **스케줄링 메커니즘**
  - cron 기반 주기적 작업 실행 (초 단위 정밀 제어)
  - 동시성 제어: 플래그를 통한 중복 실행 방지
  - 시간대 검증: 발송 허용 시간대 벗어나면 자동 종료
- **우선순위 균형 전략**
  - 즉시 발송 (immediately) 과 예약 발송 (scheduled) 교대 처리
  - 매 실행마다 우선순위 스위칭으로 한쪽 발송 유형 편중 방지
- **트랜잭션 기반 작업 처리**
  - DB 커넥션 획득 → 트랜잭션 시작
  - FOR UPDATE 락으로 작업 획득 (중복 방지)
  - 발송 성공 시 commit / 실패 시 rollback
  - 커넥션 반환으로 리소스 정리
- **결과**
  - 시간당 1,200 건 안정적 처리 (20 건/분)
  - 발송 허용 시간대 자동 준수 (야간 발송 방지)
  - 즉시/예약 발송 공평한 처리 비율 유지
