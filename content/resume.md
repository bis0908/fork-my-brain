...
저는 무언가를 만들어 **오래 안정적으로 굴러가게 하는 일**에서 가장 큰 보람을 느끼는 엔지니어입니다. 개발을 시작하기 전 9 년간 제약 자동화 장비와 발전소 설비처럼 멈추면 안 되는 시스템을 현장에서 운영했고, 그 시간이 지금도 코드를 대하는 태도의 바탕이 되어 있습니다. 화면이 잘 도는 것보다 "이게 새벽에 멈추면 어떻게 되는가"를 먼저 떠올리는 사람입니다.

제가 즐기는 일은 **문제의 진짜 원인을 측정으로 좁혀가는 과정**입니다. WebRTC 협업 서비스를 혼자 만들 때도 부하 테스트에서 예상과 다른 패턴이 보이면 추측으로 넘기지 않고 프로파일러와 통계를 붙여 끝까지 파고드는 게 재미있었습니다. 새로운 기술은 글로만 읽기보다 작게라도 직접 만들어 깨뜨려 보며 익히는 편입니다.

앞으로는 백엔드를 중심으로 **동시성·실시간 처리·성능**처럼 눈에 잘 안 보이지만 무너지면 치명적인 영역을 더 깊이 다루고 싶습니다. 혼자 3 년을 운영하며 테스트 자동화와 관측 도구의 부재를 아쉬워했던 만큼, 만들 때부터 관측 가능하고 복구 가능한 시스템을 설계하는 역량을 키우려 합니다.

멀리 보면, 현장에서 익힌 **"멈추지 않는 시스템에 대한 감각"을 팀과 제품의 신뢰성으로 옮겨놓는 사람**이 되고 싶습니다. 오래 책임질 수 있는 시스템을 함께 키우는 일에 제 강점이 가장 잘 쓰인다고 생각합니다.
...
**서버 상태 모니터링**
Heartbeat (MAC 주소 자동 등록) 기반 다중 발송 서버 상태 추적 → 운영 UI 에서 정상/장애 실시간 가시화
...
**기술 스택**: Node.js, Express, MySQL, Axios, Cheerio, AWS ELB, Nodemailer, Socket.IO, node-schedule, LLM API
