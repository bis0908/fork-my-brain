...
9년 산업 현장 운영 경험을 기반으로 안정성과 확장성을 설계하는 풀스택 엔지니어. 

시간당 1,200건 처리 이메일 마케팅 플랫폼을 3년간 무중단 운영하며 
분산 시스템 설계 역량을 검증했고, 최대 100명 동시 접속 WebRTC SFU 서버 구축으로 
실시간 통신 아키텍처 전문성을 확보.

24/7 엔터프라이즈 시스템 구축 경험으로 설계 단계부터 장애 복구와 모니터링을 고려하며, 
부하 테스트와 프로파일링을 통한 성능 최적화에 강점을 보유.
...
- **Low Spec의 클라우드 컴퓨팅 부하 테스트로 이중 병목 구조 발견**
	- (네트워크 434Mbps → CPU 50%, 참가자 3배 증가 시 Jitter 1.4배만 증가)
...
- 해결: mediasoup SFU 아키텍처 도입, Worker Least-Connection 알고리즘 할당
...
**차별화 포인트:** 9년간 24/7 산업 시스템 운영 경험 → 설계 단계부터 안정성과 장애 복구를 고려하는 개발 관점 보유

### 엔클로니 | 시스템 운영팀 대리 (2016.06 ~ 2020.12)

**제약 자동화 장비 기술 엔지니어** - 글로벌 제약사(J&J, Pfizer 등) 대상 비전 검사 시스템 납품

- **24/7 온콜 체제:** 생산 라인 긴급 장애 대응, 가동 중단 최소화
- **시스템 튜닝:** 비전 검사 임계값 최적화, 트러블슈팅 경험 (→ 성능 최적화 관점 습득)
- **글로벌 프로젝트:** 인도, 말레이시아, 일본, 독일, 중국 현장 설치 및 기술 지원
- **문서화:** GMP 기준 Validation 문서(IQ/OQ/PQ), 운영 매뉴얼 작성

### ㈜오엔테크놀러지 | 필드 엔지니어 대리 (2013.07 ~ 2015.04)

**발전소 설비 관리 시스템(CMMS) 구축** - 한국남부발전, 한국수력원자력

- **엔터프라이즈 시스템 경험:** 24/7 운영 환경의 데이터 정합성 및 가용성 요구사항 이해
- **예지보전 시스템:** 진동 모니터링 기반 장비 고장 예측, 비계획 정지 시간 최소화

### SK 커뮤니케이션즈 자회사 | 품질관리팀 사원 (2011.07 ~ 2013.06)

- 싸이월드 앱 QA, 네이트 콘텐츠 사용자 통계 관리
