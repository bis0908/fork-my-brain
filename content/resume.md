---
title: this.소개
tags:
  - resume
---
---

# 공원석

## Software Engineer

### About me

- 저는 2022년 개발자로 전향한 이후 3년간 Node.js 기반 풀스택 역량을 집중적으로 키워왔으며, 이전 9년간의 시스템 엔지니어링 경험이 개발자로서 차별화된 강점이 되고 있습니다. 산업 현장에서의 운영·구축 경험을 바탕으로, 설계와 운영 전반에 대한 깊은 이해를 갖춘 상태에서 개발에 접근하고 있습니다.

- 개발 입문 전부터 발전소 CMMS 구축, 진동측정장비 소프트웨어 엔지니어링, 웹서비스 QA 등 다양한 비개발 기술 업무를 수행하며, 시스템 전체를 조망하는 시각과 24/7 운영 환경에서 요구되는 안정성 확보 능력을 체득했습니다. 이러한 토대 덕분에 개발 단계에서부터 안정성과 유지보수성을 고려한 설계가 가능합니다.

- 또한 대학 입시 컨설팅 사이트, 시간당 1,200건을 처리하는 인플루언서 마케팅 플랫폼, WebRTC n:m 강의 시스템 등 다양한 복잡도의 프로젝트를 단기간에 수행하며 실전 중심의 성장을 이뤘습니다. 설계부터 구현, 배포까지 전 과정을 주도하며 문제 해결 속도와 품질을 동시에 높이는 역량을 확보했습니다.

- 해외 프로젝트(인도·말레이시아·일본·독일)와 산업용 시스템 운영 경험을 통해 다양한 환경과 요구사항 속에서도 유연하게 대응하는 방법을 익혔습니다. 이를 코드 설계에 녹여내어, 클라이언트 중심 사고와 장기 유지보수성을 동시에 반영하고 있습니다.

- 읽기 편한 코드와 견고한 설계를 바탕으로 안정적인 시스템을 구현하는 것을 개발 철학으로 삼고 있습니다. 프론트엔드부터 백엔드까지 아우르는 풀스택 역량과 기술 요구사항 분석 능력을 바탕으로, 클라이언트 중심의 실용적인 솔루션을 제공하겠습니다.

### 기술 스택

- Node.js
#### Frameworks / Libraries

- Next.js
- Express
#### Database

- MySQL
#### Cloud

- AWS EC2
#### Testing

- Jest


---

## 경력

### 프리랜서 (2022.12 ~ 현재)

##### 밸류앤플러스 (2025.05 ~ 2025.08, 상주 계약직)

**7개의 네이버 블로그 마케팅 자동화 도구 개발**

1. **키워드 추출기** - 블로그 카테고리별 포스트 수집, GPT 연관 키워드 생성, 네이버 랭킹 확인
2. **이미지 다운로더 & 변형 도구** - 블로그 이미지 자동 다운로드 및 3겹 레이어 합성 변형
3. **네이버 블로그 툴킷 (Python)** - 카테고리/포스트 관리, 스마트 블록 랭킹, 플레이스 리뷰 수집
4. **네이버 블로그 툴킷 (Node.js)** - Python 버전의 TypeScript/Node.js 포팅
5. **플레이스 순위 체커** - 네이버 검색 플레이스 순위 확인, BS4 기반 크롤링
6. **포스팅 재사용 검사기** - Next.js 기반 웹앱, 관리자 대시보드, 통계 분석
7. **텍스트 문단 가다듬기** - 텍스트 자동 분할 및 포맷팅 도구

##### 대학 입시 컨설팅 사이트 개발

- 기술: Node.js, express, MySQL, HTML/CSS, Vanila JS/jQuery
- 담당: 프론트엔드/백엔드 전체 개발
- 📺 [라이브 페이지](https://seedconsulting.co.kr/)

##### [[네이버 인플루언서 마케팅 플랫폼 구축|네이버 인플루언서 마케팅 플랫폼 구축]]

- 시간당 1,200건 대량 메일 발송 시스템 구축
- DB 폴링 기반 분산 처리로 중복 발송 방지 및 안정성 확보
- 상위노출 확률 기반 빅데이터 이메일 발송 시스템 개발
- 네이버 스팸 필터링 회피를 위한 딜레이 순차 발송 로직 구현
- 주요 기능: 캠페인 관리, 메일 템플릿 관리, 수신거부 시스템, 열람 통계
- 기술: Node.js, Express, Nodemailer, MySQL, CloudType 다중 배포
- 🖥️[Front Git](https://github.com/bis0908/rena-client)
- 🖥️[Server Git](https://github.com/bis0908/rena-server)

##### SFU 방식 WebRTC n:m 서버 기반 강의 시스템 경험

- 기술: Node.js, Socket.io, Mediasoup, MySQL
- 담당: 결제 시스템 구현, 프론트엔드 부분 개발

##### 인천글로벌캠퍼스(IGC) 웹사이트 유지보수

- 기술: Spring Boot, MySQL
- 담당: 부분적 기능 유지보수 및 웹 접근성

##### 수제 콜라 홍보용 랜딩 페이지 구축

- 브랜드 스토리텔링 중심의 반응형 웹 디자인
- 빠른 개발 사이클로 클라이언트 요구사항 신속 반영
- 기술: Node.js
- 📺 [라이브 페이지](https://www.tantscola.com/)

---

### 개발 정규직 근무 이력

#### 비카누스 (2022.01 ~ 2022.11)

##### Fanuc 로봇 장비 3D 스캐닝 시스템 유지보수

기술: React, Spring Boot, Redis, Docker, MySQL, Nginx

- 레거시 코드 분석을 통한 기능 개선 및 버그 수정
- React, Spring Boot, Redis, Docker 기반 풀스택 환경 경험
- 컨테이너화된 개발 환경에서의 협업 및 배포 경험

##### DAQ 데이터 수집 및 시계열 데이터 적재 시스템 유지보수

기술: React, Spring Boot, Message Broker, InfluxDB, Nginx

- 실시간 데이터 처리 파이프라인 안정성 개선
- InfluxDB 기반 시계열 데이터 관리 및 React 대시보드 개발
- Message Broker를 활용한 실시간 데이터 스트리밍 경험

---

### 토이프로젝트

#### 젠더 리빌 사이트 (완전 무료)

> 임신 중인 부모가 가족, 친구들과 함께 아기의 성별을 공개하며 축하하는 행사로, 
> 미국에서 유행하는 사회적 트렌드에서 착안하여 국내에서도 임신한 부부들을 축하하기 위해
> 온라인으로 간편히 만들고 SNS로 공유할 수 있도록 제작하였습니다.

- Cursor AI 활용으로 2일 내 빠른 프로토타이핑 및 배포 
- 4종 젠더 리빌 애니메이션, 단태아/다태아 구분, 커스텀 카운트다운 기능 구현
- Next.js, shadcn.ui, typescript
- deploy: Vercel
- 📺 [라이브페이지](https://gender-reveal-theta.vercel.app/)

---
# 비개발 경력

## 엔클로니 

>시스템 운영팀 / 대리 (2016.06 ~ 2020.12)

**정제/캡슐 외관 검사 장비 기술엔지니어**

- 국내외 장비 납품 및 유지보수, 해외 장비 데모 지원 
	- 인도, 말레이시아, 일본, 독일, 중국 등 출장
- 장비 설치부터 고객사 인도까지 전 과정 관리 및 24/7 기술지원
- 주요 국내외 전시회 참가 및 고객사 장비 데모 진행

## ㈜오엔테크놀러지

>필드 엔지니어 / 대리 (2013.07 ~ 2015.04)

**발전소 및 원자력 발전소 설비 관리 시스템 유지보수**

- 한국남부발전 삼척그린파워 설비 유지보수 관리 시스템(CMMS) 구축 및 
  AMS 소프트웨어 관리
- 한국수력원자력 각 발전소 진동측정장비 납품 및 기술지원

## SK 커뮤니케이션즈 자회사 근무

> 품질관리팀 / 사원 (2011.07 ~ 2013.06)

**웹서비스 운영 및 품질관리**

- SK 커뮤니케이션즈 싸이월드 앱 품질 테스트
- 네이트 메인 및 콘텐츠 사용자 통계 관리

---

## 교육

### 빅데이터 분석 전문가 과정 (2020.12 ~ 2021.04)

- Python, R을 활용한 빅데이터 분석
- 대용량 데이터 구축, 탐색, 분석 및 시각화 skills 습득

## 경험

- 해외 출장 경험 (인도, 말레이시아, 일본, 독일, 중국, 미국)
- 다양한 산업 분야의 기술 지원 및 시스템 구축 경험
